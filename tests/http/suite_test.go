package http

import (
	"testing"

	"github.com/Nikita-Filonov/axiom"
)

func TestSuite(t *testing.T) {
	testSuite := axiom.NewSuiteFactory(
		t,
		func() *suite { return new(suite) },
		axiom.WithSuiteConfigRunner(suiteRunner),
		axiom.WithSuiteConfigParallel(),
	)

	testSuite.Test(
		"TestCreateUser",
		(*suite).TestCreateUser,
		axiom.WithSuiteTestRunner(usersRunner),
	)

	testSuite.Test(
		"TestUpdateUser",
		(*suite).TestUpdateUser,
		axiom.WithSuiteTestRunner(usersRunner),
	)

	testSuite.Test(
		"TestGetUserMe",
		(*suite).TestGetUserMe,
		axiom.WithSuiteTestRunner(usersRunner),
	)

	testSuite.Test(
		"TestLogin",
		(*suite).TestLoginUser,
		axiom.WithSuiteTestRunner(authenticationRunner),
	)

	testSuite.Test(
		"TestLoginWithInvalidCredentials",
		(*suite).TestLoginWithInvalidCredentials,
		axiom.WithSuiteTestRunner(authenticationRunner),
	)

	testSuite.Test(
		"TestDeleteFile",
		(*suite).TestDeleteFile,
		axiom.WithSuiteTestRunner(filesRunner),
	)

	testSuite.Test(
		"TestCreateFile",
		(*suite).TestCreateFile,
		axiom.WithSuiteTestRunner(filesRunner),
	)

	testSuite.Test(
		"TestGetFile",
		(*suite).TestGetFile,
		axiom.WithSuiteTestRunner(filesRunner),
	)

	testSuite.Test(
		"TestGetFileWithUnknownID",
		(*suite).TestGetFileWithUnknownID,
		axiom.WithSuiteTestRunner(filesRunner),
	)

	testSuite.Test(
		"TestListCourses",
		(*suite).TestListCourses,
		axiom.WithSuiteTestRunner(coursesRunner),
	)

	testSuite.Test(
		"TestCreateCourseWithEmptyTitle",
		(*suite).TestCreateCourseWithEmptyTitle,
		axiom.WithSuiteTestRunner(coursesRunner),
	)

	testSuite.Test(
		"TestCreateCourse",
		(*suite).TestCreateCourse,
		axiom.WithSuiteTestRunner(coursesRunner),
	)

	testSuite.Test(
		"TestUpdateCourseWithEmptyDescription",
		(*suite).TestUpdateCourseWithEmptyDescription,
		axiom.WithSuiteTestRunner(coursesRunner),
	)

	testSuite.Test(
		"TestGetCourse",
		(*suite).TestGetCourse,
		axiom.WithSuiteTestRunner(coursesRunner),
	)

	testSuite.Test(
		"TestDeleteCourse",
		(*suite).TestDeleteCourse,
		axiom.WithSuiteTestRunner(coursesRunner),
	)

	testSuite.Test(
		"TestCreateExercise",
		(*suite).TestCreateExercise,
		axiom.WithSuiteTestRunner(exercisesRunner),
	)

	testSuite.Test(
		"TestGetExercise",
		(*suite).TestGetExercise,
		axiom.WithSuiteTestRunner(exercisesRunner),
	)

	testSuite.Test(
		"TestUpdateExercise",
		(*suite).TestUpdateExercise,
		axiom.WithSuiteTestRunner(exercisesRunner),
	)

	testSuite.Test(
		"TestDeleteExercise",
		(*suite).TestDeleteExercise,
		axiom.WithSuiteTestRunner(exercisesRunner),
	)

	testSuite.Test(
		"TestListExercises",
		(*suite).TestListExercises,
		axiom.WithSuiteTestRunner(exercisesRunner),
	)

	testSuite.Run()
}
